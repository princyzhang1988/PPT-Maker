...
2. **【阻塞确认点 1】分析方式提案**：汇总探查发现（数据形状、目标理解、
   可信结论），按 methodology §I 八法菜单提出分析方式提案，每项声明：
   方法（对比/分组/漏斗/象限/留存/RFM/显著性检验/准实验归因，
   按 §I.0 数据形状裁剪）→ 回答什么业务问题 → 数据要求是否满足
   → 预期产出与呈现形式；对比类按 §B 标注属于哪一比。
   每项同时声明「问题类型：描述/探索/推断/因果（判定依据）」（§I.0 路由）；
   预测类目标按 §I.0 规则 4 明示能力边界。目标描述太模糊时，
