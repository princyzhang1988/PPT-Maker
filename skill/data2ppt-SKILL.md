...
## 工作流（两步制：第 1 步分析过程 ⛔，第 2 步对外报告）

流程：**探查+执行分析（自动）→ 第 1 步交付「分析过程」（⛔ 用户对数据达成一致）→ 第 2 步输出对外报告（制作到交付自动）**。

**Phase A/B — 数据探查与执行分析**（自动）
1. 接收输入：数据 + 分析目的。pandas 读入规范化，直接跑探查与分析（先做数据核查：
   行列合计、口径一致性，冲突如实记录不抹平）：
   ```bash
   ...
   ```
   按 §I.0 问题类型路由选方法并逐项实跑（因果命题无对照/无干预点自动降级标暂定），
   写 `<project>/sources/facts.json`（唯一数值真源；证据不足的结论标"暂定"）。
2. 【阻塞交付点】**第 1 步：分析过程**，按序交付三部分：
   ① 针对数据的整体探查情况（概况+质量披露+整体画像）；
   ② 针对分析目的采取的分析方法及原因（§I.0 路由 + 每项方法选择依据，降级与能力边界明示）；
   ③ 整个分析结论和论据（结论句+论据链+置信标记；假设验证含「假设→方法→结果→结论」）。
   等用户对数据与结论达成一致；质疑触发补充验证。
